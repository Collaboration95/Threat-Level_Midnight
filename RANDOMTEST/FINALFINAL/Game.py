...
def Gui_List_Players():
    ...
    # Printing the list (Player Names)
    Mylabel = Label(root,text="The players are:")
    Mylabel.pack()
    
    for i in range(len(Player_Names)):
        Mylabel = Label(root,text="{} : {}".format(i+1,Player_Names[i]))
        Mylabel.pack()
...
def Gui_Sort_By_Players()->None:
    ...
    if len(Text_Container)<4:
        ...
        Mylabel.pack()
        MyButton = Button(root,text="Close this prompt",command = root.destroy)
        MyButton.pack()
        root.mainloop
        return None
...
def Game_Session(Q_list:list,User_name:str):
    ...
    for i in range(No_Of_Q):
        ...
        if (os.path.isfile(Q_list[i]["File_Path"])):
            ...
            # PhotoImage(master = canv, width = ImageSize[1], height = ImageSize[1])
